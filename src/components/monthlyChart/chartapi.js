...
export async function getIdolRank({teamName = '11-3', gender = '', pagesize = 10, cursor = ''}) {
  let query = `gender=${gender}&pageSize=${pagesize}`;
  if (cursor.length > 0) query = query.concat(`&cursor=${cursor}`);
